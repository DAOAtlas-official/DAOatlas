...
func GetArticlesList(page int, limit int, where map[string]interface{}, order string, articleTypeID int) (articlesList []model.View, err error) {
	...
	JoinDAO := db.Where("views.scenes = ?", model.BASE_POST).Where("views.typeid = ?", articleTypeID)
