...
func Auser(db *gorm.DB) bool {
	...
	if user.Id == 0 {
		userNew := new(d.User)
		userNew.Name = "dao-admin"
		userNew.Pwd = util.Md5jiayan("77DsllAq") //md5加盐后加密一下
...
func Aview(db *gorm.DB) bool {
	...
	view.Title = "欢迎使用DAO list "
	view.Typeid = 1
	view.Content = "欢迎使用DAO list"
	view.Body = "欢迎使用DAO list"
